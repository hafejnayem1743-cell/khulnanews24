...
export const Header: React.FC = () => {
  ...
  const handleScrollToContact = (e: React.MouseEvent) => {
    ...
    if (contactEl) {
      ...
      const targetPosition = contactEl.getBoundingClientRect().top + window.pageYOffset - (navHeight + 24);
      window.scrollTo({
        top: targetPosition,
        behavior: 'smooth'
      });
    ...
    <header className="w-full bg-white dark:bg-[#111827] border-b border-gray-200 dark:border-gray-800 transition-colors">
      ...
      <div className="lg:hidden px-3 pb-2 pt-0">
        ...
        >
          ...
          <span>বিজ্ঞাপন দিতে যোগাযোগ করুন: <span className="text-[#D32F2F] font-mono">{siteSettings.phone || '01306721743'}</span></span>
        </a>
      </div>
